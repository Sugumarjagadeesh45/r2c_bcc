...
const sendOTPWithNodemailer = async (email, name, otp) => {
  try {
    ...
    if (emailInfo.response && emailInfo.response.includes('development mode')) {
      ...
      return {
        success: false,
        provider: 'Nodemailer (development mode)',
        response: emailInfo
      };
    } else {
...
const sendOTPEmail = async (req, res) => {
  try {
    ...
    // Try Nodemailer first with timeout
    const nodemailerPromise = new Promise(async (resolve, reject) => {
      try {
        const startTime = Date.now();
        const result = await sendOTPWithNodemailer(email, name, otp);
    ...
    // Set a timeout for Nodemailer (2.5 seconds)
    ...
    try {
      // Race between Nodemailer and timeout
      const result = await Promise.race([nodemailerPromise, timeoutPromise]);
      
      if (result.success) {
        emailSent = true;
        emailProvider = result.provider;
        emailInfo = result.response;
        console.log(`✅ Email sent successfully via ${emailProvider} in ${deliveryTime}ms`);
      } else {
        console.log(`Nodemailer failed, trying EmailJS fallback`);
        throw new Error('Nodemailer failed');
      }
    } catch (error) {
      console.log(`Nodemailer failed or timed out, trying EmailJS fallback: ${error.message}`);
      
      // If Nodemailer fails, try EmailJS
      try {
        const startTime = Date.now();
        const result = await sendOTPWithEmailJS(email, name, otp);
        deliveryTime = Date.now() - startTime;
        ...
        emailError = emailjsError.message;
      }
    }
    ...
    const response = {
      success: true,
      message: emailSent 
        ? `OTP sent successfully via ${emailProvider}` 
    ...
    res.json({
      ...
      emailConfig: {
        nodemailer: {
          ...
          hasSmtpUser,
          hasSmtpPass,
          hasAnyConfig: hasGmailEmail && hasGmailPassword || hasSmtpHost && hasSmtpPort && hasSmtpUser && hasSmtpPass
        },
        emailjs: {
          hasEmailJS,
          emailJSServiceId: process.env.EMAILJS_SERVICE_ID ? process.env.EMAILJS_SERVICE_ID.substring(0, 5) + '...' : 'None',
...
const testEmailDelivery = async (req, res) => {
  try {
    ...
    // Test Nodemailer first with timeout
    const nodemailerPromise = new Promise(async (resolve, reject) => {
      try {
        const result = await sendOTPWithNodemailer(email, name || 'Test User', otp);
        resolve(result);
      } catch (error) {
        reject(error);
      }
    ...
    try {
      // Race between Nodemailer and timeout
      result = await Promise.race([nodemailerPromise, timeoutPromise]);
    } catch (error) {
      console.log(`Nodemailer failed or timed out, trying EmailJS fallback: ${error.message}`);
      fallbackUsed = true;
      
      // If Nodemailer fails, try EmailJS
      result = await sendOTPWithEmailJS(email, name || 'Test User', otp);
    }
